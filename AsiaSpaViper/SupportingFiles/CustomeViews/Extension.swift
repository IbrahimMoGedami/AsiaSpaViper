...
extension UITableView {
    ...
    func dequeCell<Cell : UITableViewCell>( cell : Cell.Type) -> Cell {
        
        let cellID = String(describing: Cell.self)
        let cell = self.dequeueReusableCell(withIdentifier: cellID) as? Cell ?? UITableViewCell()
        return cell as! Cell
    }
}
...
class Password {
    ...
    class func showUserPassword(textField : SkyFloatingLabelTextField , sender : UIButton) {
        ...
        if textField.text?.count != 0 {
            ...
            if textField.isSecureTextEntry == true {
                ...
                sender.setBackgroundImage(UIImage(named: "hide-password"), for: .normal)
            }
            else {
                // hide password
                textField.isSecureTextEntry = true
                sender.setBackgroundImage(UIImage(named: "view-password"), for: .normal)
            }
        }
        else {
...
extension UIButton {
    func checkUncheck(sender : UIButton , flag : Bool , img1 : String , img2 : String){
        let imgName = flag ? img1 : img2
